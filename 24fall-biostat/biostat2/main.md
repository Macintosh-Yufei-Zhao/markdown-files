...
Generally,
$$P(\xi_0,\dots, \xi_n)=P(\xi_0,\dots,\xi_{n-1})P(\xi_n|\xi_0,\dots, \xi_{n-1})=P(\xi_0)P(\xi_1|\xi_0)\dots P(\xi_n|\xi_0,\dots, \xi_{n-1})$$
...
$$P(\xi_{n+1}=j|\xi_n=i,\xi_{n-1}=i_{n-1},\dots,\xi_0=i_0)=P(\xi_{n+1}=j|\xi_n=i)$$

**Definition 2.1.5** The conditional probability $P(\xi_{n+1}=j|\xi_n=i)$ is called the **transition probability** $p_{ij}^{(n)}$ at time t, and $P^(n)=(p_{ij}^{(n)})$ is called the **transition probability matrix**(abbreviated as TPM for convenience) at time n.
...
$$\forall n;j,i+n,\dots,i_0, P(x_{n+1}=j|x_n=i_n,\dots,x_0=i_0)=P(x_{n+1}=j|x_n=i_n,x_{n-k+1}=i_{n-k+1})$$
...
**Definition 2.1.11** State $y$ of Markov chain $\{\xi_n(w):n\ge 0\}$ is called **recurrent**, if: $\Pr(\exists n>0 \text{ }s.t.\xi_n=y|\xi_0=y)=1$. The state $y$ is called a **recurrent state**, and **temporary state** for opposite.
...
**Definition 2.1.13** Further, define $f_{ij}(n)=\Pr(T_{ij}=n|\xi_0=i)$ as the **first entrance probability** from i to j.

**Definition 2.1.14** The **limited entrance probability** is the sum: $f_{ij}^*=\sum_{n=1}^\infty f_{ij}(n)=\Pr(T_{ij}<+\infty|\xi_0=i)$.
...
- Parameters $\lambda=\{\pi,A,B\}$
    ...
    - TPM $A=(a_{ij})$
    - Emission probability matrix $B=(b_{ij}),b_{il}=\Pr(Y_n=l|X_n=i)$
...
Given some HMMs and their parameters, the identification problem is a problem of Bayesian judgement with given samples. Calculate the probability of the given sample happening in every model $P(Y|\lambda)$. After normalization, the probability of the sample happening in any model $P(\lambda|Y)$:
$$P(Y=y|\lambda)=\sum_{x}P(Y=y|X=x,\lambda)P(X=x|\lambda)=\sum_{x=(x_1,\dots,x_T)}\pi(x_1)b_{x_1}(y_1)(\prod_i a_{x_1x_{i+1}}b_{x_{i+1}}(y_{i+1}))$$
...
$$\alpha_t(i)=\Pr(y_1,\dots,y_t,x_t=i|\lambda)$$
Then the hidden state of the next time step is calculated by adding all possible history paths:
$$\begin{align*}
\alpha_{t+1}(i)=&\Pr(y_1,\dots,y_{t+1},x_{t+1}=i|\lambda)\\
=&\sum_j\Pr(y_1,\dots,y_{t+1},x_t=j,x_{t+1}=i|\lambda)\\
=&\sum_j\Pr(y_1,\dots,y_t,x_t=j|\lambda)\Pr(y_{t+1},x_{t+1}=i|x_t=j,\lambda)\\
...
- Result: $\Pr(Y|\lambda)=\sum_{i=1}^N \alpha_T(i)$

### Backward algorithm

Just like Forward algorithm,
$$\begin{align*}
\beta_t(i)=&\Pr(y_{t+1},\dots,y_T|x_t=i,\lambda)\\
=&\sum_j\Pr(y_{t+1},\dots,y_T,x_{t+1}=j|x_t=i,\lambda)\\
=&\sum_j\Pr(y_{t+2},\dots,y_T|x_{t+1}=j,\lambda)\Pr(y_{t+1},x_{t+1}=j|x_t=i,\lambda)\\
...
- Result: $\Pr(Y|\lambda)=\sum_{i=1}^N\beta_1(i)\pi_ib_i(y_1)$
...
- single-point best, i.e. $X_t'=\argmax_i \Pr(X_t=i|Y)$
- path best, i.e. $\forall X,\Pr(X'|Y)\ge \Pr(X|Y)$
...
$$\begin{align*}
\gamma_t(i) &= \Pr(X_t = i | y_1, y_2, \cdots, y_T, \lambda)\\
&= \frac{\Pr(X_t = i, y_1, \cdots, y_T | \lambda)}{\Pr(y_1, \cdots, y_T | \lambda)} \\
&= \frac{\Pr(X_t = i, y_1, y_2, \cdots, y_T | \lambda)}{\sum_i \Pr(X_t = i, y_1, y_2, \cdots, y_T | \lambda)} \\
&= \frac{\Pr(y_{t+1}, \cdots, y_T | x_t = i, y_1, \cdots, y_t, \lambda) \\Pr(x_t = i, y_1, \cdots, y_t | \lambda)}{\sum_i \Pr(X_t = i, y_1, y_2, \cdots, y_T | \lambda)} \\
...
$$\delta_t(i) = \max_{x_1, \cdots, x_{t-1}} \Pr(x_1, \cdots, x_{t-1}, x_t = i, y_1, \cdots, y_t | \lambda)$$
And the recursion formula:
$$\begin{align*}
\delta_{t+1}(i) &= \max_{x_1, \cdots, x_t} \Pr(x_1, \cdots, x_t, x_{t+1} = i, y_1, \cdots, y_{t+1} | \lambda) \\
...
$$\widehat{\lambda} = \underset{\lambda}{\operatorname{Argmax}} \, \Pr(y_{1}, \cdots, y_{T} | \lambda)$$

When the state chain is given, the probability is $\Pr(y_{1}, \cdots, y_{T}, X_{1}, \cdots, X_{T} | \lambda)$; when the state chain is unknown, the probability is $\sum_{(X_1, \cdots, X_T)} \Pr(y_1, \cdots, y_T, X_1, \cdots, X_T | \lambda)$

### State chain given

$$\begin{align*}
&\Pr(y_1, \cdots, y_T; X_1, \cdots, X_T | \lambda) \\
